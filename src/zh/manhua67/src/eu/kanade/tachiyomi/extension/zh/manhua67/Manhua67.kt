package eu.kanade.tachiyomi.extension.zh.manhua67
...
class Manhua67 : HttpSource() {
    ...
    private val htmlUrl = "http://www.6mh7.com/"
    ...
    override fun mangaDetailsParse(response: Response): SManga = SManga.create().apply {
        ...
        title = document.select("div.cy_info div.cy_intro_l div.cy_title h1").text()
        thumbnail_url = document.select("div.cy_info div.cy_intro_l div.cy_info_cover img").attr("src")
        author = getMangeInfo(document.select("div.cy_info div.cy_intro_l div.cy_xinxi span"), "作者")
        artist = "Tachiyomi:ZongerZY"
        genre = getMangeInfo(document.select("div.cy_info div.cy_intro_l div.cy_xinxi span"), "标签").replace(",", ", ")
        status = if (getMangeInfo(document.select("div.cy_info div.cy_intro_l div.cy_xinxi span"), "状态").contains("连载")) 1 else 2
        description = document.select("div.cy_info div.cy_intro_l div.cy_xinxi.cy_desc p").text()
    }
    ...
    override fun chapterListParse(response: Response): List<SChapter> {
        ...
        var elements = Jsoup.parseBodyFragment(body).select("#mh-chapter-list-ol-0").select("li a")
        for (element in elements) {
            chapterList.add(SChapter.create().apply {
                name = element.select("p").text().trim()
    ...
    override fun searchMangaParse(response: Response): MangasPage {
        ...
        var mangasElements = document.select("div.cy_list_r div.cy_list_mh ul")
        var mangas = ArrayList<SManga>()
        for (mangaElement in mangasElements) {
            mangas.add(SManga.create().apply {
                title = mangaElement.select("li.title a").text()
                thumbnail_url = mangaElement.select("li a.pic img").attr("src")
                url = "${htmlUrl}${mangaElement.select("li.title a").attr("href")}"
            })
        }

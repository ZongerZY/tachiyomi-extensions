package eu.kanade.tachiyomi.extension.zh.comic88
...
class Comic88 : HttpSource() {

    override val name = "漫畫聯合國:大陆"
    override val baseUrl = "https://www.88comic.com"
    ...
    private val replaceUrl = "https://www.comicun.com"
    ...
    override fun mangaDetailsParse(response: Response): SManga = SManga.create().apply {
        ...
        thumbnail_url = document.select("div.w980.mt10.clearfix div.intro_l div.info_cover p.cover img.pic").attr("src").replace("https://img.comicun.com", imageBaseUrl)
    ...
    override fun chapterListParse(response: Response): List<SChapter> {
        ...
        for (element in elements) {
            chapterList.add(SChapter.create().apply {
                ...
                url = element.select("a").attr("href").replace(replaceUrl, baseIP[(0..2).random()])
            })
        }
        return chapterList
    }
    ...
    override fun imageUrlParse(response: Response): String {
        val body = response.body()!!.string()
        return Jsoup.parseBodyFragment(body).select("#ComicPic").attr("src").replace("https://img.comicun.com", imageBaseUrl)
    }
    ...
    override fun searchMangaParse(response: Response): MangasPage {
        ...
        for (mangaElement in mangasElements) {
            mangas.add(SManga.create().apply {
                ...
                thumbnail_url = mangaElement.select("p.fl.cover a.pic img").attr("src").replace("https://img.comicun.com", imageBaseUrl)
                url = mangaElement.select("p.fl.cover a.pic").attr("href").replace(replaceUrl, baseIP[(0..2).random()])
            })
        }

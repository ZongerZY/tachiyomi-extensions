package eu.kanade.tachiyomi.extension.zh.zongerzyxingkongmanhua
...
class Zongerzyxingkongmanhua : HttpSource() {

    override val name = "星空漫画"
    override val baseUrl = ""
    override val lang = "zh"
    override val supportsLatest = true

    private val baseUrlDomain = "https://m.acgcd.com"
    private val baseImageUrlDomain = "http://mikimiki.dailytu.com:45678"
    ...
    override fun popularMangaParse(response: Response): MangasPage {
        ...
        for (mangaElement in mangasElements) {
            mangas.add(SManga.create().apply {
                ...
                thumbnail_url = "$baseImageUrlDomain/" + mangaElement.select("a.ImgA img").attr("src").split(".com/")[1]
    ...
    override fun mangaDetailsParse(response: Response): SManga = SManga.create().apply {
        ...
        thumbnail_url = "$baseImageUrlDomain/" + document.select("div.pic img").attr("src").split(".com/")[1]
    ...
    override fun chapterListParse(response: Response): List<SChapter> {
        ...
        for (i in 0 until elements.size) {
            chapterList.add(SChapter.create().apply {
                name = elements.get(i).select("a span").text().trim()
                url = "$baseUrlDomain" + elements.get(i).select("a").attr("href")
                if (i % 5 == 0) {
                    url = "$baseUrlDomain" + elements.get(i).select("a").attr("href") + "?isAddPayImage=1"
                } else if (i == elements.size - 1) {
                    url = "$baseUrlDomain" + elements.get(i).select("a").attr("href") + "?isAddPayImage=1"
                } else {
                    url = "$baseUrlDomain" + elements.get(i).select("a").attr("href") + "?isAddPayImage=0"
                }
            })
        }
        ...
        if (urls.contains("keywords")) {
            ...
            for (mangaElement in mangasElements) {
                mangas.add(SManga.create().apply {
                    ...
                    thumbnail_url = "$baseImageUrlDomain/" + mangaElement.select("a.image-link img").attr("src").split(".com/")[1]
    ...
    override fun searchMangaRequest(page: Int, query: String, filters: FilterList): Request {
        ...
        if (query != "") {
            ...
            var url = "$baseUrlDomain/list/$params?page=$page"
